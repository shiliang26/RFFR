...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
        ...
        self.magic = torch.Tensor([0, 1, 2, 14, 15, 16, 28, 29, 30])    # Offsets against up left corner
        self.batch = torch.arange(config.batch_size, dtype=torch.long)[:, None].expand(config.batch_size, 9).contiguous().view(-1)
    ...
    def merge_output(self, pred, mask, data):
        ...
        merge = self.unpatchify(masked_patchified_data + pred * mask)
        return merge
    ...
    def random_masking(self, x, mask_ratio, block=False, block_id = None):
        ...
        # block_id = 0 if Performing random blocking
        # block_id = 1~16 if uniformly block one position.
        ...
        if block:
            if block_id is None:
                ...
                remove_row = torch.LongTensor([3 * (block_id // 4) + 1] * N).unsqueeze(1)
            ...
            if N == config.batch_size:
                ...
                batch = torch.arange(N, dtype=torch.long)[:, None].expand(N, 9).contiguous().view(-1)
    ...
    def forward_encoder(self, x, mask_ratio, block=False, block_id=None):
        ...
        if block:
            x, mask, ids_restore, row, col = self.random_masking(x, mask_ratio, block, block_id)
        else:
    ...
    def forward(self, imgs, mask_ratio=0.75, block=False, block_id=None):
        if block:
            latent, mask, ids_restore, row, col = self.forward_encoder(imgs, mask_ratio, block, block_id)
        else:
